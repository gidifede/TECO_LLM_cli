...
def _find_local_env() -> Path | None:
    """Cerca il .env nella root del progetto o nel progetto fratello test-case-evaluation."""
    project_dir = Path(__file__).resolve().parent.parent
    local = project_dir / ".env"
    if local.is_file():
        return local
    # Fallback: .env del progetto fratello test-case-evaluation
    sibling = project_dir.parent / "test-case-evaluation" / ".env"
    if sibling.is_file():
        return sibling
    return None
